...
const AddPackages = () => {
  ...
  const handleChange = (e) => {
    ...
    // Base pricing for transportation
  ...
  // Base pricing for local transportation
 ...
 const  mealPrice = 2000
  ...
  const totalPrice = (localTransportation, transportation, distance, days) => {
    let transportCost = 0;
    let localTransportCost = 0;
  

    // Assign selected transportation costs
    transportCost = transportPrices[transportation] || 0;
    localTransportCost = localTransportPrices[localTransportation] || 0;

    // Transportation cost (fixed + distance-based)
    const transportationCost = transportCost + localTransportCost + (distance * 5);

    // Accommodation and meal cost based on days
    const accommodationCost = days * accommodationPrice;
    const mealCost = days * mealPrice;

    // Final total cost
    const total = transportationCost + accommodationCost + mealCost;

    return total;
};


// Expected Output: Transportation + (3 * 5000) + (3 * 2000)
    ...
    <>
      <Box sx={{ width: '100%', display: 'flex', justifyContent: 'center', p: 3 }}>
        ...
        >
          ...
          <Fade in={true} timeout={1000}>
            <FormControl fullWidth sx={{ mb: 2 }}>
              <InputLabel>Local Transportation</InputLabel>
              <Select
          ...
          <Fade in={true} timeout={1000}>
            <TextField
              ...
              name="packagePrice"
              type="number"
              value={formData.packagePrice=totalPrice(formData.localTransportation,formData.packageTransportation, formData.distance, formData.packageDays)}
              onChange={handleChange}
              fullWidth
              variant="outlined"
              sx={{ mb: 2 }}
